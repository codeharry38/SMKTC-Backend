...
class GameWinnerDailyController extends Controller
{
    ...
    public function daily_winners($id){
        try{
            return RGameWinner::collection(GameWinner::where('game',$id)->where('is_daily', 1)->orderBy('created_at','DESC')->orderBy('position_type')->orderBy('position')->get());
        }catch(Exception $error){
             return response()->json(['action' => false, 'message' => "Somting not good!", 'error' => $error]);
        }
    }
    ...
    public function setupWinner(){
        try{
            $games = Game::where('status',true)->where('is_end',0)->where('is_daily_prize',1)->whereDate('start_at', '<=', now())->get();
        //return $games;
    ...
    private function avilablePosition($prize){
        try{
             return PrizeMeta::where('prize',$prize)->where('is_daily',1)->count(); 
        }catch(Exception $error){
            return response()->json(['action' => false, 'message' => "Somting not good!", 'error' => $error]);
        } 
    }
    ...
    private function lowPositons($prize){
        try{
             return PrizeMeta::where('prize',$prize)->where('position_type','lower')->where('is_daily',1)->count(); 
        }catch(Exception $error){
            return response()->json(['action' => false, 'message' => "Somting not good!", 'error' => $error]);
        } 
    }
    ...
    private function highPositons($prize){
        try{
             return PrizeMeta::where('prize',$prize)->where('position_type','higher')->where('is_daily',1)->count();  
        }catch(Exception $error){
            return response()->json(['action' => false, 'message' => "Somting not good!", 'error' => $error]);
        } 
    }
    ...
    private function positionDetails($prize, $position, $position_type){
        try{
            return PrizeMeta::where('prize',$prize)->where('position_type',$position_type)->where('position',$position)->where('is_daily',1)->first();  
        }catch(Exception $error){
            return response()->json(['action' => false, 'message' => "Somting not good!", 'error' => $error]);
        }
    }
